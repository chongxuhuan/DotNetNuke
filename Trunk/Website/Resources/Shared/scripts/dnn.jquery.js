...
(function ($) {
    $.fn.dnnTooltip = function (options) {
        ...
        $wrap.each(function () {
            ...
            //var dnnFormHelp = $prev.find('a.dnnFormHelp');
...
(function ($) {
        ...
        $.fn.dnnCheckbox = function (options) {
            ...
            return this.each(function () {
                ...
                if($(this).hasClass('normalCheckBox') || $(this).hasClass('normalRadioButton'))
                    return;
        ...
        $.fn.dnnHelperTip = function (options) {           
            ...
            return this.each(function () {
                ...
                $pd.hover(function(e){
                    ...
                    pd.tooltipWrapperInner.css({ left: '-10px', top: -(tooltipHeight + 30) + 'px'}).show();
                }, 
                function(){
                    hoverOnPd = false;
                    //if (!pd.tooltipWrapper.hasClass(settings.pinnedClass) && !hoverOnToolTip) {
                ...
                $pd.bind('mousemove', function(e){
                    if(hoverOnPd){
                        ...
                        // fix a bug in position relative on body while RadGrid refresh                        
...
(function($){
      ...
      $.fn.toggleErrorMessage = function(options){
            ...
            return this.each(function(){
                ...
                if(options.show){
                    var errorSpan = $('<span class="dnnFormMessage ' + options.errorCls +'">' +  options.errorMessage +'</span>');
                   dnnFormItem.append(errorSpan);
                   if(this.tagName.toLowerCase() == 'div'){
                        // customised controls
                        if($(this).hasClass('RadComboBox_Default')){
                            // RadComboBox
                        ...
                        if($(this).parent().hasClass('RadPicker_Default')){
                            // RadDate Picker
    ...
    $.dnnComboBoxLoaded = function(sender){
        if(sender.constructor.__typeName == "Telerik.Web.UI.RadComboBox") {
            $(sender._inputDomElement).closest(".RadComboBox_Default").removeAttr("style");
        }
    }

    //fix combobox hide error info
    $.dnnComboBoxHack = function(sender){        
        $(('#' + sender._clientStateFieldID).replace('_ClientState', '')).toggleErrorMessage({show: false});
    };

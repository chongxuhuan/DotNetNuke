...
<div class="vLetterSearch">
    <asp:Repeater id=rptLetterSearch Runat="server">
		<itemtemplate>
			<asp:HyperLink ID="HyperLink1" runat="server" CssClass="CommandButton" NavigateUrl='<%# FilterURL(Container.DataItem.ToString(),"1") %>' Text='<%# Container.DataItem.ToString() %>'>
			</asp:HyperLink>&nbsp;&nbsp;
		</ItemTemplate>
	</asp:Repeater>
</div>
<asp:DataGrid ID="grdPortals" runat="server" Width="100%" AutoGenerateColumns="false" CellPadding="2" GridLines="None" cssclass="dnnPortalsGrid">

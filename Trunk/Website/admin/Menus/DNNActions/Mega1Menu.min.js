...
if (!DDR.Menu.Providers.Mega1) {
    DDRjQuery(function ($) {
        DDR.Menu.Providers.Mega1 = function (jqContainer, dnnNavParams) {
            var me = this;

            me.baseConstructor(jqContainer, dnnNavParams);
        }
        DDR.Menu.Providers.Mega1.prototype = new DDR.Menu.Providers.BaseRenderer();

        DDR.Menu.Providers.Mega1.prototype.createRootMenu = function () {
            var me = this;
            ...
            me.jqContainer.queue(function () {
                me.menus.filter(function (m) {
                    ...
                    m.rendered.find("A.ddrmenuitemlevel2").matchWidths();
                });

                me.subMenus.each(function (m) {
                    m.rendered.find("a.ddrmenuitemlevel1").matchWidths();
                    m.rendered.find("a.ddrmenuitemlevel2").matchWidths();
                });
        ...
        DDR.Menu.Providers.Mega1.prototype.createRenderedMenu = function (menu) {
            var me = this;
            ...
            var result = $("<div />");
            result.addClass("ddrmenu").addClass("ddrmenulevel" + level);
        ...
        DDR.Menu.Providers.Mega1.prototype.createRenderedItem = function (item) {
            var me = this;
            ...
            var link = $("<a />").css({ "display": "block", "white-space": "nowrap" }).text(title);
            ...
            if (href) {
                ...
                link.addClass("ddrmenuitemdisabled");
            }

            if (image)
                link.prepend($("<img />").attr("src", image).css({ "border": 0, "vertical-align": "middle", "margin-bottom": ".25em" }));

            var result = link;
            result.addClass("ddrmenuitem").addClass("ddrmenuitemlevel" + level);
            if (item.isBreadcrumb || item.isSelected) {
                result.addClass("ddrmenuitemselected");
            }
            if (item.first) {
                result.addClass("ddrmenuitemfirst");
            }
            if (item.last) {
                result.addClass("ddrmenuitemlast");
            }
            if (item.first && item.last) {
                result.addClass("ddrmenuitemonly");
            }

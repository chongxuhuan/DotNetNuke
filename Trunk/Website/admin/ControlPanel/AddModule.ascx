...
<script type="text/javascript">   
    ...
    function openShareableWarning() {
        ...
        if (sharing) {
            ...
            var selectedTabId = $find('<%= PageLst.ClientID %>').get_value();
            var selectedModuleId = $find('<%= ModuleLst.ClientID %>').get_value();
...
<div id="shareableWarning">
    ...
    <br />
    <div align="center">
        <asp:LinkButton ID="cmdConfirmAdd" OnClientClick="confirmAddShareable()" runat="server"
            ResourceKey="cmdConfirmAdd" CssClass="dnnPrimaryAction" />
        <asp:LinkButton ID="cmdCancelAdd" OnClientClick="cancelAddShareable()" runat="server"
            ResourceKey="cmdCancelAdd" CssClass="dnnSecondaryAction" />
    </div>
</div>
